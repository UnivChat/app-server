...
public interface OTOChatRoomRepository extends JpaRepository<OTOChatRoom, Long> {
    ...
    // 송신자, 수신자로 채팅방 찾기
    Optional<OTOChatRoom> findBySenderAndReceiver(Member sender, Member receive);

    // 사용자로 채팅방 찾기
    // TODO: 같은 값인데, 1개로 할 수는 없을까?
    List<OTOChatRoom> findBySenderOrReceiver(Member member1, Member member2);

    boolean existsBySenderAndReceiver(Member sender, Member receive);
